...
export default function SendSuccess() {
  ...
  useEffect(() => {
    if (total > 0) notifyPaymentSent(total)
  }, [total])
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        <Text style={styles.amount}>{prettyAmount(total)}</Text>
        {config.currencyDisplay !== CurrencyDisplay.Sats && total > 0 && (
          <Text style={styles.fiat}>
            ~ {prettyNumber(toFiat(total), 2)} {config.fiat}
          </Text>
        )}
